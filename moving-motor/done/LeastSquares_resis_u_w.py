...
tension = [abs(tension[i]) for i in range(len(tension))]
tension = LPF_FILTER(0.001, tension, tension[0])
...
x = pd.DataFrame({'resistance': resis, 'tension': tension, 'velocitys': velocitys})
# x = pd.DataFrame({'resistance': resis, 'velocitys': velocitys})

plt.plot(times_fit, resis, '.', label="resis")
plt.plot(times_fit, tension, '.',label="tension")
...
for i in range(len(resis)):
    temp_lstq = k1*resis[i] + k2*tension[i] + k3*velocitys[i] + k4
