...
filename = "rolling_data_sin_sin.txt"
...
x = pd.DataFrame({'resistance': resis, 'current': current})
...
for i in range(len(resis)):
    temp_lstq = k1*resis[i] + k2*current[i] + k3
...
filenames = ["rolling_data_sin_3A.txt", "rolling_data_sin_6A.txt", "rolling_data_Sin_sampled.txt", "rolling_data_sin.txt"]
