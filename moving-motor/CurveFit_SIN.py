...
filename = "data/rolling_data_sin_3A.txt"
...
plt.plot(times_fit[:TIME1], iqs_fit[:TIME1], '.', label="iqs [Amperes]")
plt.plot(times_fit[:TIME1], vqs_fit[:TIME1], '.', label="vqs [V]")
# plt.plot(times_fit[:TIME1], np.array(velocitys_fit[:TIME1])/130 , '.', label="velocity [rad/s] / 150")
plt.plot(times_fit[:TIME1], np.array(vqs_fit[:TIME1])/ np.array(iqs_fit[:TIME1]), '.', label="resistencia [u/i]")
plt.xlabel('Time [s]')
plt.legend()
plt.show()
...
for i in range(len(iqs_fit)):
    y_estimed.append((vqs_fit[i] - kv*velocitys_fit[i]))

plt.plot(times_fit[:TIME1], y_data[:TIME1], 'g.', label="Y_data")
plt.plot(times_fit[:TIME1], y_estimed[:TIME1], '.', label="(U - kv*w)*k2")
plt.plot(times_fit[:TIME1], (np.array(y_estimed)/np.array(y_data))[:TIME1], '.', label="Resistencia [(U - kv*w)*k2 / I]")
